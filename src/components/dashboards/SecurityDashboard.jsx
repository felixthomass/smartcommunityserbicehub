...
import SecurityDutyStatus from './SecurityDutyStatus'
import ScanPassDashboard from './ScanPassDashboard'
import ModernDashboardContent from './SecurityDashboard/ModernDashboardContent'
import VisitorModule from './SecurityDashboard/VisitorModule'
import { useVisitorManagement } from './SecurityDashboard/hooks/useVisitorManagement'
import WorkerEntry from '../gate/WorkerEntry'
...
const SecurityDashboard = ({ user, onLogout, currentPage = 'dashboard' }) => {
  ...
  // Use the visitor management hook
  const {
    visitorLogs,
    filteredLogs,
    isLoading,
    ...
    stats,
    analytics,
    loadAnalytics: loadVisitorAnalytics,
    visitorForm,
    setVisitorForm,
    selectedVisitor,
    setSelectedVisitor,
  ...
  useEffect(() => {
    if (currentPage === 'scan-pass') {
      setActiveView('scan')
    } else if (currentPage === 'staff-entry') {
      setActiveView('staff-entry')
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {currentPage === 'notifications' ? (
          ...
          activeView === 'worker-entry' ? (
            <WorkerEntry onBack={() => setActiveView('list')} />
          ) :
          activeView === 'staff-entry' ? (
            <FaceAttendance onBack={() => setActiveView('list')} />
          ) :
          activeView === 'scan' ? (
            <ScanPassDashboard 
              user={user} 
          ...
          activeView === 'shifts' ? <SecurityDutyStatus user={user} /> :
          currentPage === 'dashboard' ? (
            <ModernDashboardContent 
              user={user} 
              setActiveView={setActiveView}
              onRaiseAlert={() => setActiveView('add')} // Quick fallback or can be linked to a specific type
            />
          ) :
          <VisitorModule 
            stats={stats}
            ...
            resetForm={resetForm}
            handleSubmitVisitor={handleSubmitVisitor}
            ...
            selectedVisitorForPass={selectedVisitorForPass}
            handleGeneratePass={handleGeneratePass}
            handleDownloadPDF={handleDownloadPassPDF}
            handleSendPassEmail={handleSendPassEmail}
            isSendingPassEmail={isSendingPassEmail}
          />
        }
      </div>
